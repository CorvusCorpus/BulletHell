/// @desc Stage three as it stood before Mika's fight was rewritten: a frozen
/// copy, kept on the rack for comparison.
///
/// **This is a snapshot of `stage_sanctum` at commit 09469bb, renamed and
/// otherwise untouched.** The working stage is having its boss rebuilt as
/// fifteen attacks, one slot at a time, and the seven placeholder attacks it
/// had are being replaced as that happens. This card keeps the whole of the
/// old stage playable -- its waves, the Proctor, and Mika's seven -- so there
/// is something to play the new ones against, and so nothing about the old
/// fight has to be recovered from git to be looked at.
///
/// **Every name in it carries `old_` or `OLD_`**, because GML functions and
/// macros are global and the working stage still defines the originals. The
/// copy calls nothing in `stage_sanctum`, so the working stage can change
/// anything it likes -- its helpers included -- without this one moving.
///
/// **It cannot touch the save.** `id` is the empty string, and
/// `progress_record` refuses an empty id; a run of this card reaches that call
/// exactly as a run of a real stage does, so the guard is load-bearing here in
/// a way it is not for the drafting table or the review card.
///
/// **Deleting this file and its line in `rack_list` removes the whole card**,
/// along with `test_old_sanctum` in `selftest`. That is the plan once the
/// working stage's slots are all filled -- see "Mika's fifteen" in
/// `CLAUDE.md`.
///
/// The notes below are the originals, renamed with everything else. Where
/// they talk about "this stage" they mean the stage as it was.
...
/// @desc The card. A whole stage in every way the run cares about, and in no
///       way the save does.
///
///       **`id` is empty where the original's was `gilded_sanctum`.** Keeping
///       the real id would file a clear of the old fight against the working
///       stage, and a stage marked beaten because somebody played the
///       placeholder version is exactly the permanent, silent mistake
///       `practice_new` keeps its own id empty to avoid.
///
///       `old_draft` is what the rack reads to say what the card is, through
///       `stage_is_old_draft`.
function old_stage_sanctum_def() {
    return {
        ...
        // `turned` because he is fought after the hall opens; see
        // `practice_begin`.
        bosses: [
...
/// @desc Is this rack entry the old draft of stage three?
///
///       **Read through `[$ ]`, because every other entry lacks the field** --
///       the trap `stage_is_draft` and `stage_is_preview` are written around.
